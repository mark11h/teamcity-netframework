...
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'teamcity_netframework')
BEGIN
	CREATE DATABASE [teamcity_netframework]	
END
GO
...
IF SUSER_ID('teamcity_netframework_user') IS NULL
BEGIN
	CREATE LOGIN [teamcity_netframework_user] WITH PASSWORD=N'G?HdcS2D8#w83)nFx', DEFAULT_DATABASE=[teamcity_netframework], DEFAULT_LANGUAGE=[us_english], CHECK_EXPIRATION=OFF, CHECK_POLICY=OFF
END
GO

USE [teamcity_netframework]
GO

IF DATABASE_PRINCIPAL_ID('teamcity_netframework_user') IS NULL
BEGIN
	CREATE USER [teamcity_netframework_user] FOR LOGIN [teamcity_netframework_user] WITH DEFAULT_SCHEMA=[dbo]	
	exec sp_addrolemember 'db_owner', 'teamcity_netframework_user';
END
GO
